...
//       .then(res => {
//         if (res.data.status !== "ZERO_RESULTS" && res.data.results[0].name.replace(/ /g, "").toLowerCase() === userInputLowerCase) { // (res.data.status !== "ZERO_RESULTS") {
//           inputType = "eat";
//           skip = true;
//         }
...
//         if (res.data.Response !== "False" && res.data.Title.replace(/ /g, "").toLowerCase() === userInputLowerCase) { // (res.data.response !== 'False') {
//           inputType = "watch";
//           skip = true;
//         }
...
//           return;
//         }
//         if (res.data.totalItems !== 0 && res.data.items[0].volumeInfo.title.replace(/ /g, "").toLowerCase() === userInputLowerCase) { // (res.data.totalItems !== 0) {
//           inputType = "read";
//           skip = true
//           return;
//         }
  ...
  makeAPICalls: function (userInput) {
    ...
    return axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=49.2827,-123.1207&radius=2500&keyword=${userInputSerialized}&type=cafe&key=${googleAPIKey}`)
      ...
      .then(res => {
        ...
        if (res.data.Response !== "False" && ((res.data.Title.replace(/ /g, "").toLowerCase() === userInputLowerCase) || (res.data.Title.replace(/ /g, "").toLowerCase().startsWith(userInputLowerCase)))) {
          inputType = "watch";
          skip = true;
        }
      ...
      .then(res => {
        ...
        if (res.data.totalItems !== 0 && res.data.items){
          ...
          if ((first.volumeInfo.title.replace(/ /g, "").toLowerCase() === userInputLowerCase) || (first.volumeInfo.title.replace(/ /g, "").toLowerCase().startsWith(userInputLowerCase))) {
            inputType = 'read';
            skip = true
            return;
          }
