  ...
  makeAPICalls: function (userInput) {
    ...
    return axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=49.2827,-123.1207&radius=2500&keyword=${userInputSerialized}&type=cafe&key=${googleAPIKey}`)
      ...
      .then(res => {
        ...
        if (res.data.totalItems !== 0 && ((res.data.items[0].volumeInfo.title.replace(/ /g, "").toLowerCase() === userInputLowerCase)) || (res.data.items[0].volumeInfo.title.replace(/ /g, "").toLowerCase().startsWith(userInputLowerCase))) { // (res.data.totalItems !== 0) {
          inputType = 'read'
          return inputType;
        }
